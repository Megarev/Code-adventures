...
public:
	...
	void Move(const std::vector<Box>& boxes, Opponent& o, const Level& level) {
	...
	void Logic(Level& itemMap, bool& isRun, bool isWinTileActive, const std::vector<ToggleTile>& tiles) {
		...
		case 'W':
			...
			for (auto& tile : tiles) {
				if ((!isWinTileActive && tile.GetIsTileActive())) {
					isWin = true;
					isTileIntrepreted = true;
				}
			}

			if (!isTileIntrepreted && isWinTileActive) {
				isWin = true;
			}
...
public:
	PlayState(const sf::Vector2u& size)
		...
		levelManager.SetIndex(9);
		textManager.SetIndex(9);
	...
	void Render(sf::RenderWindow& window) override {
		...
		for (auto& box : boxes) {
			...
			bool isActive = false;
			for (auto& tile : tiles) {
				if (tile.GetPosition() == box.GetPosition()) break;
			}
...
private:
	...
	void ToggleScreenMode() {
		...
		if (isFullScreen) {
			Window.create({ windowSize.x, windowSize.y }, windowTitle, sf::Style::Fullscreen);
		}
		else {
