...
namespace gui {
	...
	private:
		...
		void Reset(int x, int y) {
			if (!onPress) {
				button.setTextureRect(sf::IntRect(x * buttonSize, y * buttonSize, buttonSize, buttonSize));
			}
		}
	...
	public:
		SpriteButton() {}
		SpriteButton(int x, int y, int size, const sf::Vector2f& pos)
			: buttonSize(size) {
			button.setPosition(pos);
			button.setTexture(AssetHolder::Get().GetTexture("buttons"));
			SetRect(x, y);
